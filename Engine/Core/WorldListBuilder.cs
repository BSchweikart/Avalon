   ...
   { 
      ...
      public static void Generate()
      {
            ...
            using (StreamReader reader = File.OpenText(@"../../.../Engine/Docs/Monsters.csv"))
            {
                ...
                {
                    string[] line = ReadNextLine(reader); 
                    WorldGenerator.monsters.Add(new Monster(int.Parse(line[0]),line[1],int.Parse(line[2]),int.Parse(line[3]),int.Parse(line[4]),int.Parse(line[5]), int.Parse(line[6]), int.Parse(line[7]), bool.Parse(line[8]), bool.Parse(line[9]),Faction.Admin));
                }
            }
            ...
            using (StreamReader reader = File.OpenText(@"../../.../Engine/Docs/Rooms.csv"))
            {
                while (!reader.EndOfStream)
                {
                    ...
                    WorldGenerator.Location.Add
                        (
                        new Room(int.Parse(line[0]),line[1], line[2], int.Parse(line[3]), int.Parse(line[4]), int.Parse(line[5]), int.Parse(line[6]), int.Parse(line[7]))
                        );
                }
            }
